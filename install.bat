...
if exist "%~dp0zae.py" (
    ...
    python -c "import urllib.request, os, ssl; ctx = ssl.create_default_context(); ctx.check_hostname = False; ctx.verify_mode = ssl.CERT_NONE; req = urllib.request.Request('https://raw.githubusercontent.com/rootscripts/ZAE/main/zae.py', headers={'User-Agent': 'Mozilla/5.0'}); data = urllib.request.urlopen(req, context=ctx).read(); open(os.path.expanduser('~/.local/bin/zae.py'), 'wb').write(data)"
)
